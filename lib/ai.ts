...
5. 只要有任何條文能回答問題的「一部分」，就必須回答該部分，並可在 caveat 說明未涵蓋的限制；僅在所有條文與問題完全無關時，才只輸出一行：{"t":"caveat","text":"原因"}`;
...
1. query：把問題改寫成「法條會使用的語言」的檢索語句（30 字內），優先使用「請求權基礎」術語（所有物返還請求權、無權占有、侵權行為損害賠償、不當得利、給付遲延、契約解除等）。
